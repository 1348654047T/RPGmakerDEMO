 ...
 * 2.你需要在插件中一个个配置魔法圈绑定到动画上面。
 *  （绑定后，配置的动画和rmmv动画同时播放，rmmv动画你需要手动设置额外延时时间）
 * 3.你可以通过插件指令关闭所有动画魔法圈，做到长时间持续的魔法防御被打断。
...
//			._drill_anim_circles_charBack 图片后面层（临时）
//			._drill_parent_sprite 确定跟随位移
//	
//		【设置 战斗单位+动画id 的魔法圈消失】绕等级★★★
...
　　var DrillUp = DrillUp || {}; 
	...
	DrillUp.anim_circles_max = 200;
	DrillUp.anim_circles = [];
	
	for (var i = 0; i < DrillUp.anim_circles_max; i++) {
		if( DrillUp.parameters['魔法圈-' + String(i+1) ] != "" ){
			DrillUp.anim_circles[i] = JSON.parse(DrillUp.parameters['魔法圈-' + String(i+1) ]);
			DrillUp.anim_circles[i]['id'] = i+1;
			DrillUp.anim_circles[i]['visible'] = String(DrillUp.anim_circles[i]["初始是否显示"] || "true") == "true";
			DrillUp.anim_circles[i]['anim'] = Number(DrillUp.anim_circles[i]["绑定的动画"]);
			DrillUp.anim_circles[i]['src_img'] = String(DrillUp.anim_circles[i]["资源-魔法圈"]);
			DrillUp.anim_circles[i]['x'] = Number(DrillUp.anim_circles[i]["平移-魔法圈 X"]);
			DrillUp.anim_circles[i]['y'] = Number(DrillUp.anim_circles[i]["平移-魔法圈 Y"]);
			DrillUp.anim_circles[i]['blendMode'] = Number(DrillUp.anim_circles[i]["混合模式"]);
			DrillUp.anim_circles[i]['rotate'] = Number(DrillUp.anim_circles[i]["旋转速度"]);
			DrillUp.anim_circles[i]['anim_index'] = String(DrillUp.anim_circles[i]["动画层级"]);
			DrillUp.anim_circles[i]['zIndex'] = Number(DrillUp.anim_circles[i]["图片层级"]);
			
			DrillUp.anim_circles[i]['delay'] = Number(DrillUp.anim_circles[i]["出现延迟"]);
			DrillUp.anim_circles[i]['birth'] = Number(DrillUp.anim_circles[i]["出现时长"]);
			DrillUp.anim_circles[i]['birthMode'] = String(DrillUp.anim_circles[i]["出现模式"] || "横向展开");
			DrillUp.anim_circles[i]['sustain'] = Number(DrillUp.anim_circles[i]["持续时长"]);
			DrillUp.anim_circles[i]['death'] = Number(DrillUp.anim_circles[i]["消失时长"]);
			DrillUp.anim_circles[i]['deathMode'] = String(DrillUp.anim_circles[i]["消失模式"] || "普通淡出消失");
			
			DrillUp.anim_circles[i]['scale_x'] = Number(DrillUp.anim_circles[i]["缩放 X"] || 1.0);
			DrillUp.anim_circles[i]['scale_y'] = Number(DrillUp.anim_circles[i]["缩放 Y"] || 1.0);
			DrillUp.anim_circles[i]['skew_x'] = Number(DrillUp.anim_circles[i]["斜切 X"] || 0);
			DrillUp.anim_circles[i]['skew_y'] = Number(DrillUp.anim_circles[i]["斜切 Y"] || 0);
		}else{
			DrillUp.anim_circles[i] = [];
		}
	}
...
// * 播放中的动画魔法圈 - 容器
//=============================================================================
	DrillUp.anim_playing_aCircles = [];	//全局临时存储正在动画中播放的魔法圈（该操作可能不安全，但是目前没有别的方法）
	
	//设置 所有动画id 的魔法圈消失
	DrillUp.drill_aCircles_setDeathById = function(tar_id) {
		for(var i in DrillUp.anim_playing_aCircles){
			var temp = DrillUp.anim_playing_aCircles[i];
	...
	DrillUp.drill_aCircles_updateDelete = function() {
		for (var i in DrillUp.anim_playing_aCircles ) {	
			...
			if( temp['_drill_time_all'] <= temp['_drill_cur_time']){
				DrillUp.anim_playing_aCircles.splice(i,1);
				delete temp;
			}
			break;
		}
...
// * 插件指令
//=============================================================================
var _drill_anim_circles_pluginCommand = Game_Interpreter.prototype.pluginCommand;
Game_Interpreter.prototype.pluginCommand = function(command, args) {
	_drill_anim_circles_pluginCommand.call(this, command, args);
	if (command === '>动画魔法圈') {
		if(args.length == 4){
			var temp1 = Number(args[1]) - 1;
			var type = String(args[3]);
			if (type === '显示') {
				$gameSystem._drill_anim_circles_visible[temp1] = true;
			}
			if (type === '隐藏') {
				$gameSystem._drill_anim_circles_visible[temp1] = false;
			}
		}
	}
	if (command === '>播放中的动画魔法圈') {
		if(args.length == 4){
			...
			if (type === '立即显示') {
				for(var i in DrillUp.anim_playing_aCircles){
					if( DrillUp.anim_playing_aCircles[i].id == temp_id ){
						DrillUp.anim_playing_aCircles[i].visible = true;
					}
				}
			}
			if (type === '立即隐藏') {
				for(var i in DrillUp.anim_playing_aCircles){
					if( DrillUp.anim_playing_aCircles[i].id == temp_id ){
						DrillUp.anim_playing_aCircles[i].visible = false;
					}
				}
			}
			if (type === '立即消失') {
...
// ** 存储变量初始化
//=============================================================================
var _drill_animCircle_sys_initialize = Game_System.prototype.initialize;
Game_System.prototype.initialize = function() {
    _drill_animCircle_sys_initialize.call(this);
	this._drill_anim_circles_visible = [];
	for(var i = 0; i< DrillUp.anim_circles.length ;i++){
		this._drill_anim_circles_visible[i] = DrillUp.anim_circles[i]['visible'];
	}
};	
...
// * 动画-初始化
//==============================
var _drill_anim_circles_initMembers = Sprite_Animation.prototype.initMembers;
Sprite_Animation.prototype.initMembers = function() {
	_drill_anim_circles_initMembers.call(this);
...
// * 动画-设置
//==============================
var _drill_anim_circles_setup = Sprite_Animation.prototype.setup;
Sprite_Animation.prototype.setup = function(target, animation, mirror, delay) {
	
	if( !this._drill_anim_circles_charBack ){	//图像后面层（预置，后续在父类中重置取出）
		this._drill_anim_circles_charBack = new Sprite();
		this.addChild(this._drill_anim_circles_charBack);
	}
	if( !this._drill_anim_back ){		//动画后面层
		this._drill_anim_back = new Sprite();
		this.addChild(this._drill_anim_back);
	}
    _drill_anim_circles_setup.call(this,target, animation, mirror, delay);
		...
		for (var i = 0; i < DrillUp.anim_circles.length; i++) {			
			...
			if(this._animation.id == anim_data['anim'] ){
				...
				if( anim_data['anim_index'] == "在动画前面" ){
					...
					this._drill_anim_circles_charBack.addChild(temp);
				}
...
// * 动画-图像后面层 -战斗层定义
//==============================
var _drill_anim_circles_createEnemies = Spriteset_Battle.prototype.createEnemies;
Spriteset_Battle.prototype.createEnemies = function() {
	...
	_drill_anim_circles_createEnemies.call(this);	
};

//==============================
// * 动画-图像后面层 -地图层定义
//==============================
var _drill_anim_circles_createCharacters = Spriteset_Map.prototype.createCharacters;
Spriteset_Map.prototype.createCharacters = function() {
	...
	_drill_anim_circles_createCharacters.call(this);
};
//==============================
// * 动画-图像后面层 -添加动画
//==============================
var _drill_anim_circles_startAnimation = Sprite_Base.prototype.startAnimation;
Sprite_Base.prototype.startAnimation = function(animation, mirror, delay) {
    _drill_anim_circles_startAnimation.call(this,animation, mirror, delay);
	...
	if(	last_sprite._drill_anim_circles_charBack != undefined &&
		last_sprite._drill_anim_circles_charBack.children.length != 0){
		...
		if( this.constructor.name == "Sprite_Enemy" 
			...
			if( this.parent != undefined && this.parent.parent != undefined && this.parent.parent.parent != undefined &&
				...
				var anims = last_sprite._drill_anim_circles_charBack.children;
...
// * 动画-播放中
//==============================
var _drill_anim_circles_isPlaying = Sprite_Animation.prototype.isPlaying;
Sprite_Animation.prototype.isPlaying = function() {
    if( this._drill_duration > 0 ){
		return true;
	}
	return _drill_anim_circles_isPlaying.call(this);
};

//==============================
// * 动画-帧刷新
//==============================
var _drill_anim_circles_update = Sprite_Animation.prototype.update;
Sprite_Animation.prototype.update = function() {
	this._drill_duration_decreased = false;		//减一锁，确保多次继承后，减一后，不会继续执行减一方法。
	_drill_anim_circles_update.call(this);
...
// * 动画-移除（空指针优化）
//==============================
var _drill_anim_circles_remove = Sprite_Animation.prototype.remove;
Sprite_Animation.prototype.remove = function() {
	if( this._target != undefined ){
		_drill_anim_circles_remove.call(this);
	}else{
		if (this.parent) {
...
// ** 魔法圈外部控制（消除sprite）
//=============================================================================
var _drill_anim_circles_timer_update = Game_Timer.prototype.update;
Game_Timer.prototype.update = function(sceneActive) {
    _drill_anim_circles_timer_update.call(this,sceneActive);
...
Drill_AnimationCircle_Sprite.prototype.initialize = function(animation,settings) {
	...
	this.anchor.x = 0.5;
	this.anchor.y = 0.5;
	this.visible = $gameSystem._drill_anim_circles_visible[settings['id']-1];
...
Drill_AnimationCircle_Sprite.prototype.drill_createCircle = function() {
	...
	var temp_sprite_bitmap = new Sprite(ImageManager.loadAnimation(this._drill_data['src_img']));
