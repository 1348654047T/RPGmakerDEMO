...
Window_Base.prototype.drawActorMood = function(actor, x, y, width) {
    ...
    this.resetTextColor();
    $gameVariables._data[2] = actor._actorId;
    this.drawText("ムード：" + $gameParty.npc()["mood"] + " / 15" , x, y +10 +120, width + 100);
    this.drawText("中出しした回数：" + $gameParty.npc()["nakadasi"] , x, y + 30 +10 +120, width + 100);
    this.drawText("孕ませた回数：" + $gameParty.npc()["harami"] , x, y + 60 +10 +120, width + 100);
    if ($gameParty.npc()["bp_plus_a"] >= 1) { this.drawText("V強化：こうげき" , x, y + 90 +10 +120, width + 100);}
    if ($gameParty.npc()["bp_plus_b"] >= 1) { this.drawText("V強化：まほう" , x, y + 120 +10 +120, width + 100);}
...
Window_Base.prototype.drawActorLevel = function(actor, x, y) {
    ...
    var rankup_drw = ""
    if ($dataClasses[actor._classId].meta.rankup_id != null) { var rankup_drw = " [wait rank_up]" 
    if ($dataClasses[actor._classId].meta.rankup_lv <= actor._level) {var rankup_drw = " [can rank_up]"  }
    }
