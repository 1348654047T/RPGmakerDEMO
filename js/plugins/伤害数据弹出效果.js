...
//=============================================================================
/*:
 * @plugindesc (v1.1)[v1.1]  敌人 - 伤害数字弹出效果（属性颜色版）(添加暴击数值代替暴击闪光)
 * @author Moghunter （Drill_up优化+Lbt改版）
 *
 * @param 持续时间
 ...
 *
 * @param 资源-伤害数字效果
 * @desc 伤害数字效果的图片资源。和Damage.png格式一样。（大小可变）
 * @default ["Damage-火","Damage-冰"]
 * @require 1
 * @dir img/system/
 * @type file[]
 *
 ...
 * -----------------------------------------------------------------------------
 * ----可选设定
 * 你可以设置指定的技能、物品技能的伤害数字具有属性颜色效果。
 *
 * 技能注释：<伤害数字效果:1>
 *
 * 设置后，技能将按照指定的文件进行变化。
 * 数字1对应的 资源-伤害数字效果 中的第1个伤害图片资源。
 *
 * -----------------------------------------------------------------------------
 * ----可选设定
...
　　var Moghunter = Moghunter || {}; 
	...
	if(Moghunter.parameters['资源-伤害数字效果'] != "" ){
		Moghunter.dmgp_colors = JSON.parse(Moghunter.parameters['资源-伤害数字效果']);
	}else{
		Moghunter.dmgp_colors = [];
	}
...
// ** 技能属性 与 伤害图片绑定
//=============================================================================	
var _drill_dmgpop_initialize = Game_System.prototype.initialize;
Game_System.prototype.initialize = function() {
	_drill_dmgpop_initialize.call(this);
	this._drill_dmgpop_skill_color = [];
	this._drill_dmgpop_item_color = [];
	for( var i = 0; i < $dataItems.length; i++ ){	//物品
		if( $dataItems[i] == null ){this._drill_dmgpop_item_color[i] = 0;continue;}
		var note = String($dataItems[i].note);
		var re_color = /<伤害数字效果:([^<>]*?)>/; 		//正则获取（返回数组，第二个为匹配内容）
		var color = (note.match(re_color)) || [];
		if(color != "" && color != [] ){
			this._drill_dmgpop_item_color[i] = Number(color[1]);
		}else{
			this._drill_dmgpop_item_color[i] = 0;
		}
	}
	for( var i = 0; i < $dataSkills.length; i++ ){	//技能
		if( $dataSkills[i] == null ){this._drill_dmgpop_skill_color[i] = "";continue;}
		var note = String($dataSkills[i].note);
		var re_color = /<伤害数字效果:([^<>]*?)>/; 		
		var color = (note.match(re_color)) || [];
		if(color != "" && color != [] ){
			this._drill_dmgpop_skill_color[i] = Number(color[1]);
		}else{
			this._drill_dmgpop_skill_color[i] = 0;
		}
	}
	 this._drill_dmgpop_colors = [];
	 this._drill_dmgpop_colors.push( ImageManager.loadSystem('Damage') );
	 for(var i = 0; i< Moghunter.dmgp_colors.length; i++){
		this._drill_dmgpop_colors.push( ImageManager.loadSystem(Moghunter.dmgp_colors[i]) );
	 }
};
...
Game_Action.prototype.repeatTargets = function(targets) {
    ...
    for (var i = 0; i < targets.length; i++) {
		...
		target._drill_skill_from = this._item ;
        if (target) {
...
Sprite_Damage.prototype.initialize = function() {
	 ...
	 this._damage = [0,0];
	 this._drill_dmgpop_color_cur = 0;
	 this._drill_dmgpop_color = 0;
	 this._drill_dmgpop_colors = $gameSystem._drill_dmgpop_colors;
};
...
Sprite_Damage.prototype.setup = function(target) {
	if(target._drill_skill_from){
		var item_class = target._drill_skill_from['_dataClass'];
		var item_id = target._drill_skill_from['_itemId'];
		if( item_class == "skill" ){
			var e_num = $gameSystem._drill_dmgpop_skill_color[item_id];
		}else{
			var e_num = $gameSystem._drill_dmgpop_item_color[item_id];
		}
		this._drill_dmgpop_color = e_num;
		this._damageBitmap = this._drill_dmgpop_colors[ this._drill_dmgpop_color ];
	}else{
		this._damageBitmap = this._drill_dmgpop_colors[ 0 ];
	}
