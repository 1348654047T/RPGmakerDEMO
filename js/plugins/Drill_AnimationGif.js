 ...
 * 2.你需要在插件中一个个配置GIF绑定到动画上面。
 *  （绑定后，配置的动画和rmmv动画同时播放，rmmv动画你需要手动设置额外延时时间）
 * 3.你可以通过插件指令关闭所有动画GIF，做到长时间持续的魔法防御被打断。
...
//插件记录：
//
//		该插件的原理模式与菜单GIF有较大不同，虽然配置原理相似。
//		与 MOG_BattleHud 和 Drill_BattleCamera 有关联，用于定位第一人称下的动画位置。
//		
//		在circle的基础上添加了gif播放功能，保留了3d效果。
//
...
　　var DrillUp = DrillUp || {}; 
	...
	DrillUp.anim_GIFs_max = 200;
	DrillUp.anim_GIFs = [];
	
	for (var i = 0; i < DrillUp.anim_GIFs_max; i++) {
		if( DrillUp.parameters['GIF-' + String(i+1) ] != "" ){
			DrillUp.anim_GIFs[i] = JSON.parse(DrillUp.parameters['GIF-' + String(i+1) ]);
			DrillUp.anim_GIFs[i]['id'] = i+1;
			DrillUp.anim_GIFs[i]['visible'] = String(DrillUp.anim_GIFs[i]["初始是否显示"] || "true") == "true";
			DrillUp.anim_GIFs[i]['anim'] = Number(DrillUp.anim_GIFs[i]["绑定的动画"]);
			DrillUp.anim_GIFs[i]['x'] = Number(DrillUp.anim_GIFs[i]["平移-GIF X"]);
			DrillUp.anim_GIFs[i]['y'] = Number(DrillUp.anim_GIFs[i]["平移-GIF Y"]);
			DrillUp.anim_GIFs[i]['blendMode'] = Number(DrillUp.anim_GIFs[i]["混合模式"]);
			DrillUp.anim_GIFs[i]['rotate'] = Number(DrillUp.anim_GIFs[i]["旋转速度"]);
			DrillUp.anim_GIFs[i]['anim_index'] = String(DrillUp.anim_GIFs[i]["动画层级"]);
			DrillUp.anim_GIFs[i]['zIndex'] = Number(DrillUp.anim_GIFs[i]["图片层级"]);
			
			DrillUp.anim_GIFs[i]['delay'] = Number(DrillUp.anim_GIFs[i]["出现延迟"]);
			DrillUp.anim_GIFs[i]['birth'] = Number(DrillUp.anim_GIFs[i]["出现时长"]);
			DrillUp.anim_GIFs[i]['birthMode'] = String(DrillUp.anim_GIFs[i]["出现模式"] || "横向展开");
			DrillUp.anim_GIFs[i]['sustain'] = Number(DrillUp.anim_GIFs[i]["持续时长"]);
			DrillUp.anim_GIFs[i]['death'] = Number(DrillUp.anim_GIFs[i]["消失时长"]);
			DrillUp.anim_GIFs[i]['deathMode'] = String(DrillUp.anim_GIFs[i]["消失模式"] || "普通淡出消失");
			
			DrillUp.anim_GIFs[i]['src_img'] = JSON.parse(DrillUp.anim_GIFs[i]["资源-GIF"]);
			DrillUp.anim_GIFs[i]['interval'] = Number(DrillUp.anim_GIFs[i]["帧间隔"]);
			DrillUp.anim_GIFs[i]['back_run'] = String(DrillUp.anim_GIFs[i]["是否倒放"] || "false") == "true";
			DrillUp.anim_GIFs[i]['src_bitmaps'] = [];
			
			DrillUp.anim_GIFs[i]['scale_x'] = Number(DrillUp.anim_GIFs[i]["缩放 X"] || 1.0);
			DrillUp.anim_GIFs[i]['scale_y'] = Number(DrillUp.anim_GIFs[i]["缩放 Y"] || 1.0);
			DrillUp.anim_GIFs[i]['skew_x'] = Number(DrillUp.anim_GIFs[i]["斜切 X"] || 0);
			DrillUp.anim_GIFs[i]['skew_y'] = Number(DrillUp.anim_GIFs[i]["斜切 Y"] || 0);
...
// * 播放中的动画GIF - 容器
//=============================================================================
	DrillUp.anim_playing_aGIFs = [];	//全局临时存储正在动画中播放的GIF（该操作可能不安全，但是目前没有别的方法）
	
	//设置 所有动画id 的GIF消失
	DrillUp.drill_aGIFs_setDeathById = function(tar_id) {
		for(var i in DrillUp.anim_playing_aGIFs){
			var temp = DrillUp.anim_playing_aGIFs[i];
	...
	DrillUp.drill_aGIFs_updateDelete = function() {
		for (var i in DrillUp.anim_playing_aGIFs ) {	
			...
			if( temp['_drill_time_all'] <= temp['_drill_cur_time']){
				DrillUp.anim_playing_aGIFs.splice(i,1);
				delete temp;
			}
			break;
		}
...
// * 插件指令
//=============================================================================
var _drill_anim_GIFs_pluginCommand = Game_Interpreter.prototype.pluginCommand;
Game_Interpreter.prototype.pluginCommand = function(command, args) {
	_drill_anim_GIFs_pluginCommand.call(this, command, args);
	if (command === '>动画GIF') {
		if(args.length == 4){
			var temp1 = Number(args[1]) - 1;
			var type = String(args[3]);
			if (type === '显示') {
				$gameSystem._drill_anim_GIFs_visible[temp1] = true;
			}
			if (type === '隐藏') {
				$gameSystem._drill_anim_GIFs_visible[temp1] = false;
			}
		}
	}
	if (command === '>播放中的动画GIF') {
		if(args.length == 4){
			...
			if (type === '立即显示') {
				for(var i in DrillUp.anim_playing_aGIFs){
					if( DrillUp.anim_playing_aGIFs[i].id == temp_id ){
						DrillUp.anim_playing_aGIFs[i].visible = true;
					}
				}
			}
			if (type === '立即隐藏') {
				for(var i in DrillUp.anim_playing_aGIFs){
					if( DrillUp.anim_playing_aGIFs[i].id == temp_id ){
						DrillUp.anim_playing_aGIFs[i].visible = false;
					}
				}
			}
			if (type === '立即消失') {
...
// ** 存储变量初始化
//=============================================================================
var _drill_animGIF_sys_initialize = Game_System.prototype.initialize;
Game_System.prototype.initialize = function() {
    _drill_animGIF_sys_initialize.call(this);
	this._drill_anim_GIFs_visible = [];
	for(var i = 0; i< DrillUp.anim_GIFs.length ;i++){
		this._drill_anim_GIFs_visible[i] = DrillUp.anim_GIFs[i]['visible'];
	}
};	
...
// * 动画-初始化
//==============================
var _drill_anim_GIFs_initMembers = Sprite_Animation.prototype.initMembers;
Sprite_Animation.prototype.initMembers = function() {
	_drill_anim_GIFs_initMembers.call(this);
...
// * 动画-设置
//==============================
var _drill_anim_GIFs_setup = Sprite_Animation.prototype.setup;
Sprite_Animation.prototype.setup = function(target, animation, mirror, delay) {
	
	if( !this._drill_anim_GIFs_charBack ){	//图像后面层（预置，后续在父类中重置取出）
		this._drill_anim_GIFs_charBack = new Sprite();
		this.addChild(this._drill_anim_GIFs_charBack);
	}
	if( !this._drill_anim_back ){		//动画后面层
		this._drill_anim_back = new Sprite();
		this.addChild(this._drill_anim_back);
	}
    _drill_anim_GIFs_setup.call(this,target, animation, mirror, delay);
		...
		for (var i = 0; i < DrillUp.anim_GIFs.length; i++) {			
			...
			if(this._animation.id == anim_data['anim'] ){
				...
				if( anim_data['anim_index'] == "在动画前面" ){
					...
					this._drill_anim_GIFs_charBack.addChild(temp);
				}
...
// * 动画-图像后面层 -战斗层定义
//==============================
var _drill_anim_GIFs_createEnemies = Spriteset_Battle.prototype.createEnemies;
Spriteset_Battle.prototype.createEnemies = function() {
	...
	_drill_anim_GIFs_createEnemies.call(this);	
};

//==============================
// * 动画-图像后面层 -地图层定义
//==============================
var _drill_anim_GIFs_createCharacters = Spriteset_Map.prototype.createCharacters;
Spriteset_Map.prototype.createCharacters = function() {
	...
	_drill_anim_GIFs_createCharacters.call(this);
};
//==============================
// * 动画-图像后面层 -添加动画
//==============================
var _drill_anim_GIFs_startAnimation = Sprite_Base.prototype.startAnimation;
Sprite_Base.prototype.startAnimation = function(animation, mirror, delay) {
    _drill_anim_GIFs_startAnimation.call(this,animation, mirror, delay);
	...
	if(	last_sprite._drill_anim_GIFs_charBack != undefined &&
		last_sprite._drill_anim_GIFs_charBack.children.length != 0){
		...
		if( this.constructor.name == "Sprite_Enemy" 
			...
			if( this.parent != undefined && this.parent.parent != undefined && this.parent.parent.parent != undefined &&
				...
				var anims = last_sprite._drill_anim_GIFs_charBack.children;
...
// * 动画-播放中
//==============================
var _drill_anim_GIFs_isPlaying = Sprite_Animation.prototype.isPlaying;
Sprite_Animation.prototype.isPlaying = function() {
    if( this._drill_duration > 0 ){
		return true;
	}
	return _drill_anim_GIFs_isPlaying.call(this);
};

//==============================
// * 动画-帧刷新
//==============================
var _drill_anim_GIFs_update = Sprite_Animation.prototype.update;
Sprite_Animation.prototype.update = function() {
	this._drill_duration_decreased = false;		//减一锁，确保多次继承后，减一后，不会继续执行减一方法。
	_drill_anim_GIFs_update.call(this);
...
// * 动画-移除（空指针优化）
//==============================
var _drill_anim_GIFs_remove = Sprite_Animation.prototype.remove;
Sprite_Animation.prototype.remove = function() {
	if( this._target != undefined ){
		_drill_anim_GIFs_remove.call(this);
	}else{
		if (this.parent) {
...
// ** GIF外部控制（消除sprite）
//=============================================================================
var _drill_anim_GIFs_timer_update = Game_Timer.prototype.update;
Game_Timer.prototype.update = function(sceneActive) {
    _drill_anim_GIFs_timer_update.call(this,sceneActive);
...
Drill_AnimationGIF_Sprite.prototype.initialize = function(animation,settings) {
	...
	this.anchor.x = 0.5;
	this.anchor.y = 0.5;
	this.visible = $gameSystem._drill_anim_GIFs_visible[settings['id']-1];
...
Drill_AnimationGIF_Sprite.prototype.drill_createGIF = function() {
	...
	for(var j = 0; j < this._drill_data['src_img'].length ; j++){
		this._drill_data['src_bitmaps'].push(ImageManager.loadAnimation(this._drill_data['src_img'][j]));
	}
