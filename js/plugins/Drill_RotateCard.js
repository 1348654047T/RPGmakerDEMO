 ...
 *     （作用的界面不影响其界面的其他相关插件。）
 * 被扩展：
 *   - Drill_X_ItemImage 主菜单-物品+技能详细图片[扩展]
 *     通过该插件，卡片中的图标可以换成高清图片。
 *
 * -----------------------------------------------------------------------------
 * ----插件冲突
 * 该插件与yep物品核心插件相冲突，使得旋转卡牌可能无法显示。
 *
 * -----------------------------------------------------------------------------
 * ----关联文件
 ...
 * 2.每个界面的卡片类型(装备类/技能类/物品类)都可以不一样，这里可以
 * 对界面单独设置卡片类型。
 * 3.这里设置的是卡片类型，不是对应的每个物品，对每个物品的改变图像
 ...
 */
    ...
    DrillUp.RCard_btn_key = JSON.parse( DrillUp.parameters['切换按钮'] || [] );
    DrillUp.RCard_useSceneItem = String(DrillUp.parameters['是否启用物品界面卡片'] || "true") === "true";
    DrillUp.RCard_item_offset_fix = String(DrillUp.parameters['是否固定物品界面坐标'] || "true") === "true" ;
    DrillUp.RCard_item_offsetX = Number(DrillUp.parameters['平移-物品界面窗口 X'] || 0);
    DrillUp.RCard_item_offsetY = Number(DrillUp.parameters['平移-物品界面窗口 Y'] || 0);
    DrillUp.RCard_useSceneSkill = String(DrillUp.parameters['是否启用技能界面卡片'] || "true") === "true";
    DrillUp.RCard_skill_offset_fix = String(DrillUp.parameters['是否固定技能界面坐标'] || "true") === "true" ;
    DrillUp.RCard_skill_offsetX = Number(DrillUp.parameters['平移-技能界面窗口 X'] || 0);
    DrillUp.RCard_skill_offsetY = Number(DrillUp.parameters['平移-技能界面窗口 Y'] || 0);
    DrillUp.RCard_useSceneEquip = String(DrillUp.parameters['是否启用装备界面卡片'] || "true") === "true";
    DrillUp.RCard_equip_offset_fix = String(DrillUp.parameters['是否固定装备界面坐标'] || "true") === "true" ;
    DrillUp.RCard_equip_offsetX = Number(DrillUp.parameters['平移-装备界面窗口 X'] || 0);
    DrillUp.RCard_equip_offsetY = Number(DrillUp.parameters['平移-装备界面窗口 Y'] || 0);
    DrillUp.RCard_useSceneShop = String(DrillUp.parameters['是否启用商店界面卡片'] || "true") === "true";
    DrillUp.RCard_shop_offset_fix = String(DrillUp.parameters['是否固定商店界面坐标'] || "true") === "true" ;
    DrillUp.RCard_shop_offsetX = Number(DrillUp.parameters['平移-商店界面窗口 X'] || 0);
    DrillUp.RCard_shop_offsetY = Number(DrillUp.parameters['平移-商店界面窗口 Y'] || 0);
	DrillUp.RCard_useSceneBattle = String(DrillUp.parameters['是否启用战斗界面卡片'] || "true") === "true";
    DrillUp.RCard_battle_offset_fix = String(DrillUp.parameters['是否固定战斗界面坐标'] || "true") === "true" ;
    DrillUp.RCard_battle_offsetX = Number(DrillUp.parameters['平移-战斗界面窗口 X'] || 0);
    DrillUp.RCard_battle_offsetY = Number(DrillUp.parameters['平移-战斗界面窗口 Y'] || 0);
	DrillUp.RCard_useSceneFTKR = String(DrillUp.parameters['是否启用技能树界面卡片'] || "true") === "true";
    DrillUp.RCard_FTKR_offset_fix = String(DrillUp.parameters['是否固定FTKR技能树界面坐标'] || "true") === "true" ;
    DrillUp.RCard_FTKR_offsetX = Number(DrillUp.parameters['平移-FTKR技能树界面窗口 X'] || 0);
    DrillUp.RCard_FTKR_offsetY = Number(DrillUp.parameters['平移-FTKR技能树界面窗口 Y'] || 0);
	
	DrillUp.RCard_list = [];
	if( DrillUp.parameters['物品卡片设置'] != "" ){
		DrillUp.RCard_list[1] = JSON.parse(DrillUp.parameters['物品卡片设置']);
	}else{
		DrillUp.RCard_list[1] = "";
	}
	if( DrillUp.parameters['技能卡片设置'] != "" ){
		DrillUp.RCard_list[2] = JSON.parse(DrillUp.parameters['技能卡片设置']);
	}else{
		DrillUp.RCard_list[2] = "";
	}
	if( DrillUp.parameters['装备卡片设置'] != "" ){
		DrillUp.RCard_list[3] = JSON.parse(DrillUp.parameters['装备卡片设置']);
	}else{
		DrillUp.RCard_list[3] = "";
	}
	if( DrillUp.parameters['商店卡片设置'] != "" ){
		DrillUp.RCard_list[4] = JSON.parse(DrillUp.parameters['商店卡片设置']);
	}else{
		DrillUp.RCard_list[4] = "";
	}
	if( DrillUp.parameters['战斗卡片设置'] != "" ){
		DrillUp.RCard_list[5] = JSON.parse(DrillUp.parameters['战斗卡片设置']);
	}else{
		DrillUp.RCard_list[5] = "";
	}
	if( DrillUp.parameters['技能树卡片设置'] != "" ){
		DrillUp.RCard_list[6] = JSON.parse(DrillUp.parameters['技能树卡片设置']);
	}else{
		DrillUp.RCard_list[6] = "";
	}
	//alert(JSON.stringify(DrillUp.RCard_list[1]));
	
	
	DrillUp.RCard_is_in_window = 0;
...
// ** ImageManager
//=============================================================================
ImageManager.loadMenuCard = function(filename) {
    return this.loadBitmap('img/menus_card/', filename, 0, true);
};
...
Window_Selectable.prototype.drill_isCardSwitchTriggered = function() {
	for (var i=0; i<DrillUp.RCard_btn_key.length; i++) {	//按键判断
		var key = DrillUp.RCard_btn_key[i];
...
// * 初始化
//=============================
Scene_Base.prototype.drill_createRotateCard = function() {	//界面只有一个卡片

	for (var i = 0; i < DrillUp.RCard_list.length ; i++ ) {	//配置初始化
		if(DrillUp.RCard_list[i] == null || DrillUp.RCard_list[i] == ""){ continue ;}
		var card = DrillUp.RCard_list[i];
		card['img_list'] = [];
		card['img_list'][0] = ImageManager.loadMenuCard( card['资源-默认卡片正面'] );
		for (var j = 1; j <= 12 ; j++ ) {
			card['img_list'][j] = ImageManager.loadMenuCard( card['资源-卡片正面' + String(j)] );
		}
		card['img_back_list'] = [];
		card['img_back_list'][0] = ImageManager.loadMenuCard( card['资源-默认卡片背面'] );
		for (var j = 1; j <= 12 ; j++ ) {
			card['img_back_list'][j] = ImageManager.loadMenuCard( card['资源-卡片背面' + String(j)] );
		}
	...
	//alert( DrillUp.RCard_list[1]['img_list'] );
...
Scene_Item.prototype.create = function() {
	...
	if (DrillUp.RCard_useSceneItem){ this.drill_createRotateCard();}
	DrillUp.RCard_is_in_window = 1;
};
//=============================
// * 技能界面
//=============================
...
Scene_Skill.prototype.create = function() {
	...
	if (DrillUp.RCard_useSceneSkill){ this.drill_createRotateCard();}
	DrillUp.RCard_is_in_window = 2;
};
//=============================
// * 装备界面
//=============================
...
Scene_Equip.prototype.create = function() {
	...
	if (DrillUp.RCard_useSceneEquip){ this.drill_createRotateCard();}
	DrillUp.RCard_is_in_window = 3;
};
//=============================
// * 商店界面
//=============================
...
Scene_Shop.prototype.create = function() {
	...
	if (DrillUp.RCard_useSceneShop){ this.drill_createRotateCard();}
	DrillUp.RCard_is_in_window = 4;
};
//=============================
// * 战斗界面
//=============================
...
Scene_Battle.prototype.create = function() {
	...
	if (DrillUp.RCard_useSceneBattle){ this.drill_createRotateCard();}
	DrillUp.RCard_is_in_window = 5;
};
//=============================
...
if(Imported.FTKR_STS && FTKR && FTKR.STS) {
	...
	Scene_STS.prototype.createSkillTreeWindow = function() {
		...
		if (DrillUp.RCard_useSceneFTKR){ this.drill_createRotateCard();}
		DrillUp.RCard_is_in_window = 6;
	};
...
Sprite_RCard.prototype.update = function() {
	var card = DrillUp.RCard_list[DrillUp.RCard_is_in_window];
...
Sprite_RCard.prototype.refreshPosition = function(rect, padding) {
	...
	if( DrillUp.RCard_is_in_window === 1 ){
		if( DrillUp.RCard_item_offset_fix){
			this._org_x = DrillUp.RCard_item_offsetX;
			this._org_y = DrillUp.RCard_item_offsetY;
		}else{
			this._org_x += DrillUp.RCard_item_offsetX;
			this._org_y += DrillUp.RCard_item_offsetY;
		}
	}else if( DrillUp.RCard_is_in_window === 2 ){
		if( DrillUp.RCard_skill_offset_fix ){
			this._org_x = DrillUp.RCard_skill_offsetX;
			this._org_y = DrillUp.RCard_skill_offsetY;
		}else{
			this._org_x += DrillUp.RCard_skill_offsetX;
			this._org_y += DrillUp.RCard_skill_offsetY;
		}
	}else if( DrillUp.RCard_is_in_window === 3 ){
		if( DrillUp.RCard_equip_offset_fix ){
			this._org_x = DrillUp.RCard_equip_offsetX;
			this._org_y = DrillUp.RCard_equip_offsetY;
		}else{
			this._org_x += DrillUp.RCard_equip_offsetX;
			this._org_y += DrillUp.RCard_equip_offsetY;
		}
	}else if( DrillUp.RCard_is_in_window === 4 ){
		if( DrillUp.RCard_shop_offset_fix ){
			this._org_x = DrillUp.RCard_shop_offsetX;
			this._org_y = DrillUp.RCard_shop_offsetY;
		}else{
			this._org_x += DrillUp.RCard_shop_offsetX;
			this._org_y += DrillUp.RCard_shop_offsetY;
		}
	}else if( DrillUp.RCard_is_in_window === 5 ){
		if( DrillUp.RCard_battle_offset_fix ){
			this._org_x = DrillUp.RCard_battle_offsetX;
			this._org_y = DrillUp.RCard_battle_offsetY;
		}else{
			this._org_x += DrillUp.RCard_battle_offsetX;
			this._org_y += DrillUp.RCard_battle_offsetY;
		}
	}else if( DrillUp.RCard_is_in_window === 6 ){
		if( DrillUp.RCard_FTKR_offset_fix  ){
			this._org_x = DrillUp.RCard_FTKR_offsetX;
			this._org_y = DrillUp.RCard_FTKR_offsetY;
		}else{
			this._org_x += DrillUp.RCard_FTKR_offsetX;
			this._org_y += DrillUp.RCard_FTKR_offsetY;
		}
